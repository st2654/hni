...
public class RegisterServiceIntTest {
    ...
    public void setUp() {
        ...
        state = new RegistrationState(EventName.REGISTER, SESSION_ID, PHONE_NUMBER, null, RegistrationStep.STATE_REGISTER_START);
    ...
    public void testStartRegister() throws Exception {
        String returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "message"));
    ...
    public void testRegisterWorkFlow() throws Exception {
        ...
        String returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "ENROLL"));
        Assert.assertEquals("Welcome to Hunger Not Impossible! Msg & data rates may apply. "
                + "Any information you provide here will be kept private. "
                + "Reply with PRIVACY to learn more. Let's get you registered. What's your first name?", returnString);
        // first name
        returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "john"));
        Assert.assertEquals("Thanks " + "john" + ". What's your last name?", returnString);
        // last name
        returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "doe"));
        ...
        // email
        returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "johndoe@gmail.com"));
        ...
        // confirm email
        returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "1"));
        Assert.assertEquals("Please enter the 6 digit authorization code provided to you for this program.", returnString);
        // auth code
        returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "123456"));
        ...
        returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "111111"));
    ...
    public void testCorrectEmailOnce() throws Exception {
        ...
        Assert.assertEquals("Okay! I have " + "johndoe@gmail.com" + " as your email address. "
                + "Is that correct? Reply 1 for yes and 2 for no", returnString);
        // reject email
        returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "2"));
        Assert.assertEquals("So what's your email address?", returnString);
        // email
        returnString = registerService.handleEvent(Event.createEvent(SESSION_ID, PHONE_NUMBER, "johndoe2@gmail.com"));
